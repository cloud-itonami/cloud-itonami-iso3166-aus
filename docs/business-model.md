# Business Model: Independent Public-Sector Market-Entry & Procurement Compliance Service — Australia

## Classification

- Repository: `cloud-itonami-iso3166-aus`
- ISO 3166: `AUS` (Australia)
...
- Social impact: [:indigenous-procurement-policy-access :public-spend-transparency :cross-border-friction-reduction]

## Customer

- an already-incorporated `cloud-itonami-cofog-{code}` /
  `cloud-itonami-isco-{code}` / `cloud-itonami-unspsc-{segment}` /
  `cloud-itonami-{ISIC}` operator wanting to bid on an Australian
  public contract
...
- registration walkthrough for AusTender, the Australian Government's procurement information system
- business/tax registration checklist: an Australian Business Number (ABN) via the Australian Business Register (ABR), plus ASIC company registration where incorporating locally; GST registration with the Australian Taxation Office (ATO) where the turnover threshold is met
- local-content / preferential-procurement navigation: the Indigenous Procurement Policy (IPP), which sets mandatory minimum-purchase targets and set-asides for Indigenous-owned businesses on Commonwealth contracts
...
- this service does **not** provide legal or tax advice; characterization
  ...
  routes to Australian-licensed counsel or a registered agent
...
- **`cloud-itonami-cofog-{code}`**: a jurisdiction-agnostic operator
  ...
  blueprint AND this one to operate in Australia).
