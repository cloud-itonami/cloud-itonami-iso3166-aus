(ns marketentry.store
  "SSoT for the AUS market-entry compliance actor, behind a `Store`
...
(defn demo-data
  ...
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Systems AU" :portal "AusTender"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-au-presence? true :has-au-presence? true
             :requires-abn? true :abn-verified? true
             :drafted? false :submitted? false
             :jurisdiction "AUS" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "AusTender"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-au-presence? true :has-au-presence? true
             :requires-abn? true :abn-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Minami Systems" :portal "AusTender"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "AUS" :status :intake}
    "eng-4" {:id "eng-4" :operator "Higashi Export" :portal "AusTender"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-au-presence? true :has-au-presence? false
             :requires-abn? true :abn-verified? true
             :drafted? false :submitted? false
             :jurisdiction "AUS" :status :intake}
    "eng-5" {:id "eng-5" :operator "Nishi Logistics" :portal "AusTender"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-au-presence? true :has-au-presence? true
             :requires-abn? true :abn-verified? false
             :drafted? false :submitted? false
             :jurisdiction "AUS" :status :intake}
    "eng-6" {:id "eng-6" :operator "Chuo Civic Tech" :portal "AusTender"
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               requires-au-presence? has-au-presence?
                               requires-abn? abn-verified?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? requires-au-presence?)  (assoc :engagement/requires-au-presence? requires-au-presence?)
    (some? has-au-presence?)       (assoc :engagement/has-au-presence? has-au-presence?)
    (some? requires-abn?)    (assoc :engagement/requires-abn? requires-abn?)
    (some? abn-verified?)    (assoc :engagement/abn-verified? abn-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/requires-au-presence? :engagement/has-au-presence?
   :engagement/requires-abn? :engagement/abn-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :requires-au-presence? (boolean (:engagement/requires-au-presence? m))
     :has-au-presence? (boolean (:engagement/has-au-presence? m))
     :requires-abn? (boolean (:engagement/requires-abn? m))
     :abn-verified? (boolean (:engagement/abn-verified? m))
...
(defn kotobase-store
  ...
  [{:keys [db-api seq-fn]}]
  (->KotobaseStore (persist/store {:db-api db-api :actor "marketentry-aus"})
