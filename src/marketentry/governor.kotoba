(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether an Australia-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Australia-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-au-presence?
                                       true` (near-universal for Australia
                                       ...
                                       Grounded in Australian Business Number
                                       entity or authorized local presence for
                                       many Commonwealth procurements
                                       (contracting authorities / AusTender).
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(ABN/統一資格/AusTender登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- au-presence-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-au-presence? true`, INDEPENDENTLY verify
  `:has-au-presence?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-au-presence?` ground truth (most Australia public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-au-presence? e))
                 (not (true? (:has-au-presence? e))))
        [{:rule :au-presence-missing
          :detail (str subject " はオーストラリア居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- abn-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-abn? true`, INDEPENDENTLY check
  `:abn-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-abn? e))
                 (not (true? (:abn-verified? e))))
        [{:rule :abn-unverified
          :detail (str subject " はABN確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (au-presence-missing-violations request st)
                           (engagement-fee-mismatch-violations request st)
                           (abn-unverified-violations request st)
