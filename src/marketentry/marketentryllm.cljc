(ns marketentry.marketentryllm
  ...
  the AUS public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-au-presence?=" (:has-au-presence? e)
                        " abn-verified?=" (:abn-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-au-presence? e))
                              (:has-au-presence? e))
                          (or (not (:requires-abn? e))
                              (:abn-verified? e)))
                   0.9 0.3)}))
